# Polyatomics_Quiz_with_Tracking
...
# Display High Score in the sidebar
    ...
    with st.form(key='quiz_form', clear_on_submit=True):
        ...
        if submit:
            if user_input.replace(" ", "").lower() == correct_ans.replace(" ", "").lower():
                st.toast(f"Nice! {current_ion} is correct.", icon="✅")
                st.session_state.score += 1
            else:
                st.error(f"Not quite. {current_ion} is {correct_ans}")
